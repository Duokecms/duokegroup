...
<view class="tn-flex tn-flex-row-between tn-margin-top" :style="{paddingTop: vuex_custom_bar_height + 'px'}">
				...
				<view class="tn-strip-bottom">
					...
					<view class="tn-flex tn-flex-wrap tn-margin-bottom">
						<block v-for="(item, index) in news" :key="index">
							<view class="" style="width: 25%;"@click="xuan(item)">
			 ...
			 async flowGroup() {
			 	...
			 	if (result.statusCode == 200) {
			 		if (result.data.code == 200) {			
			 			that.news = result.data.data.data;			 
			 		} else {
			 			uni.showToast({
			 				icon: 'none',
